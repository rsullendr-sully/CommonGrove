...
function WindSculpture({ position, scale, index, reducedMotion, mature, woodTexture }: { position: [number, number, number]; scale: number; index: number; reducedMotion: boolean; mature: boolean; woodTexture: THREE.Texture }) {
  ...
  return (
    <group position={position} scale={scale}>
      <mesh position={[0, 1.2, 0]} castShadow><cylinderGeometry args={[0.065, 0.09, 2.4, 12]} /><CraftedWoodMaterial map={woodTexture} color="#b59c79" /></mesh>
      <group ref={wheel} position={[0, 2.4, 0]} rotation={[0, 0, index * .3]}>
        ...
        {[0, 1, 2, 3].map((blade) => (
          <group key={blade} rotation={[0, 0, blade * Math.PI / 2]}>
            <mesh position={[0, .5, 0]} rotation={[0, .2, .18]} castShadow><SoftBoxGeometry args={[.28, .78, .065]} /><meshStandardMaterial color={blade % 2 ? '#88b8ac' : '#e0b66d'} roughness={.85} /></mesh>
          </group>
        ))}
      </group>
    </group>
...
export default function JourneyGrowth({ visit, choice, reducedMotion, woodTexture }: { visit: Visit; choice: GardenChoice | null; reducedMotion: boolean; woodTexture: THREE.Texture }) {
      ...
      <group position={[PAVILION_CENTER.x, PAVILION_BASE_Y, PAVILION_CENTER.z]}>
        ...
        {Array.from({ length: visit }, (_, index) => (
          <mesh key={`book-${index}`} position={[1.88, .68 + index * .085, .25]} rotation={[0, index * .12, 0]} castShadow>
            <SoftBoxGeometry args={[.48, .08, .58]} /><meshStandardMaterial color={['#af7961', '#698b83', '#d1af77', '#8f7c99'][index]} roughness={.95} />
          </mesh>
        ))}
        {visit >= 3 && [-1, 0, 1].map((x) => (
      ...
      {choice === 'workshop' && (
        <group position={[-13, 0, 11]}>
          {([[-.95, 1.12, 1.82], [1.7, .25, 1.3], [0, .28, -1.82]] as [number, number, number][]).slice(0, growth.inventionCount).map((position, index) => (
            <WindSculpture key={index} position={position} scale={[.45, .6, .65][index]} index={index} reducedMotion={reducedMotion} mature={visit >= 3} woodTexture={woodTexture} />
          ))}
          {visit === 4 && (
            <group position={[0, 1.98, 1.34]}>
              <mesh><SoftBoxGeometry args={[3.2, .065, .06]} /><meshStandardMaterial color="#8b6b46" /></mesh>
